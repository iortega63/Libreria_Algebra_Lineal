...
class Matrix:
    ...
    def T(self) -> Matrix:
        if not hasattr(self, "_T"):
            self._T = Matrix(zip(*self._rowvectors))
        return self._T

    @property
    def determinant(self) -> float:
        if not hasattr(self, "_det"):
            self._det = self.determinant_rec()
        return self._det

    def determinant_rec(self) -> float:
        if self.shape == (2,2):
            ...
            while i < len(self):
                ...
                for j in range(1,len(self)):
                    ...
                    for k in range(len(self)):
                        if i != k:
                            lista.append(self[j][k])
                    matrix.append(lista)
                signo = signo * -1
                det = det + self[0][i] * signo * Matrix(matrix).determinant_rec()
                i += 1            
        self._det = det
        return self._det
...
def scale(m: Matrix, k: t.Union[int, float]) -> Matrix:
    """Scale matrix m by k. """
    mul = []
    for i in m._rowvectors:
        mul.append((i.__rmul__(k)))
    return Matrix(mul)
...
def add(m1: Matrix, m2: Matrix) -> Matrix:
    ...
    if m1.shape != m2.shape:
        msg = "Matrix must have the same shape, got {} and {}"
        raise ValueError(msg.format(m1.shape, m2.shape))
    
    x = []
    for i,j in zip(m1._rowvectors , m2._rowvectors):
        x.append(i.__add__(j))
    return Matrix(x)


def subtract(m1: Matrix, m2: Matrix) -> Matrix:
    """Substracts the second matrix from the first one. """
    return m1.__add__(m2.__neg__())    
...
def matrix_multiply(m1: Matrix, m2: Matrix) -> Matrix:
    ...
    lista = []
    for i in m2.columnvectors:
        lista.append(vector_multiply(m1,i))
    return Matrix(lista).T
